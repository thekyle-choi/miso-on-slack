...
import { Bold, Italic, Underline, Strikethrough, Link2, List, ListOrdered, Code, Plus, Type, Smile, AtSign, Video, Mic, Zap, ChevronDown, Send, X } from "lucide-react"
...
type SlackInputProps = {
  onMessageSent?: (message: string, result?: string, taskId?: string) => void
  channelName?: string
}

export function SlackInput({ onMessageSent, channelName = "gs-holdings-52g-salesforce-slack" }: SlackInputProps) {
  ...
  const callMisoWorkflow = useCallback(async (query: string, taskId: string, imageFile?: File) => {
    ...
    try {
      ...
      if (onMessageSent) {
        // 에러를 콜백으로 전달하여 로딩 메시지를 에러 메시지로 교체
        const now = new Date()
        ...
        // 에러 메시지를 result 대신 특별한 형식으로 전달
        onMessageSent("", `ERROR:${errorMessage}`, taskId)
      }
    }
  }, [onMessageSent])
  ...
  const handleSendMessage = useCallback(() => {
    ...
    if (trimmedMessage.startsWith("/tbm")) {
      ...
      if (onMessageSent) {
        onMessageSent(trimmedMessage, undefined, taskId) // 먼저 사용자 메시지 전송
      }

      // 입력 필드 먼저 초기화 (블로킹 없이 즉시)
      setMessage("")
      ...
      // 비동기 작업을 백그라운드에서 실행 (await 없이)
      callMisoWorkflow(query, taskId, selectedImage || undefined).catch((err) => {
  ...
  }, [message, selectedImage, callMisoWorkflow, onMessageSent, handleRemoveImage, generateTaskId])
